 ...
 */
public class ActorRuntimeGrpc extends AbstractActorRuntime {
   ...
   */
  private static volatile AbstractActorRuntime instance;
   ...
   */
  ActorRuntimeGrpc() throws IllegalStateException {
   ...
   * @throws IllegalStateException If cannot instantiate Runtime.
   */
  private ActorRuntimeGrpc(ManagedChannel channel) throws IllegalStateException {
    this(channel, buildDaprClient(channel));
  }

  /**
   * Constructor with dependency injection, useful for testing. This should not be
   * called directly.
   *
   * @param channel    GRPC managed channel to be closed (or null).
   ...
   */
  private ActorRuntimeGrpc(ManagedChannel channel, DaprClient daprClient) throws IllegalStateException {
    if (instance != null) {
  ...
  public static AbstractActorRuntime getInstance() {
    if (instance == null) {
      synchronized (ActorRuntimeGrpc.class) {
        if (instance == null) {
          instance = new ActorRuntimeGrpc();
        }
      }
    }
   ...
   *
   * @param clazz        The type of actor.
   * @param actorFactory An optional factory to create actors. This can be used
   ...
   *
   * @param clazz            The type of actor.
   * @param actorFactory     An optional factory to create actors. This can be
   ...
   * @param actorId         Actor id for the actor for which method will be
   *                        invoked.
